...
get.sheer.trans.branch <- function(file_infos, use.depth, sel.depth, cell_type_list){
  #file_infos = filter_file
  #use.depth = "lca_depth_norm"
  #sel.depth = 0.4
  #cell_type_list <- c("OFF", "ON")
  gene_list <- unique(file_infos$gene)
  sheer.T.pergene <- mclapply(seq(length(gene_list)),function(g){
    ...
    sheer.T.perfile <- mclapply(seq(length(file_list)),function(f){
      ...
      #file_path <- "CD20090806_ceh-43_RW10154_L1.csv"
      ...
      df_tree.new1 <- df_tree.new %>% mutate(D.u=treeDe$u[match(.$parent, treeDe$subtree_root)],
                                             D.u2=treeDe$u[match(.$node, treeDe$subtree_root)],
                                             D.u2=ifelse(is.na(D.u2), 0, D.u2),
                                             D.delta=D.u-D.u2)
      df_tree.new1 %>% filter(D.u <= sel.depth) %>% select(type.combined) %>% pull() %>% table()
      
      #==3 calculate sheer trans
      all_sheer_matrix_list <- list()
      depth_values <- unique(df_tree.new1$D.u)
      # Only consider depths less than or equal to fi_depth
      depth_values <- depth_values[depth_values <= sel.depth]
      
      lapply(seq_along(depth_values), function(depth_index){
        #depth_index <- 3
        current_depth <- depth_values[depth_index]
        print(current_depth)
        sheer_subset0 <- df_tree.new1 %>% filter(D.u == current_depth) %>% group_by(type.combined) %>%
          add_count(type.combined, name="num") %>% ungroup() %>%  select(5:last_col()) %>% unique() %>% as.data.frame()
        sheer_subset <- sheer_subset0 %>% select(-c(D.u2, D.delta)) %>% unique() %>%
          group_by(state.parent) %>% mutate(freq=num/sum(num)) %>% ungroup() %>% as.data.frame()
        
        # Build an initial C(u) count matrix with rows and columns corresponding to cell_type_list
        sheer_matrix <- matrix(0, nrow = length(cell_type_list), ncol = length(cell_type_list))
        colnames(sheer_matrix) <- cell_type_list
        rownames(sheer_matrix) <- cell_type_list
        
        lapply(seq_len(nrow(sheer_subset)), function(s) {
          #s <- 1
          cell_1 <- sheer_subset$state.parent[s]
          cell_2 <- sheer_subset$state.node[s]
          freq <- sheer_subset$freq[s]
          sheer_matrix[cell_2, cell_1] <<- freq
          return(NULL)
        })
        print(sheer_matrix)
        
        #=3.2 scale sheer trans base on delta depth
        if (all(sheer_matrix == 0) || (sum(sheer_matrix == 0) == 3 && 
                                       sum(sheer_matrix == 1) == 1 && 
                                       length(sheer_matrix) == 4)) {
          sheer_matrix_norm <- sheer_matrix
        } else {
          #if (all(sheer_matrix[, 2] == 0)) {
          #  sheer_matrix[, 2] <- c(0, 1)
          #}
          Tmp <- replace(sheer_matrix, is.nan(sheer_matrix), 0)
          # Perform eigen decomposition on matrix
          ...
          # 
          delta_d <- mean(sheer_subset0$D.delta)
          # Compute the transition matrix by adjusting eigenvalues based on the current depth
          sheer_matrix_norm <- V %*% diag(eigen_values^(1/delta_d)) %*%  solve(V)
          #sheer_matrix_norm <- V %*% diag(eigen_values^(1/delta)) %*% t(V)
          # Use only the real part and set negative or NaN values to 0
          sheer_matrix_norm <- Re(sheer_matrix_norm)
          sheer_matrix_norm[sheer_matrix_norm < 0 | is.na(sheer_matrix_norm)] <- 0
          sheer_matrix_norm[is.nan(sheer_matrix_norm)] <- 0
          # Normalize each column of the transition matrix so that each column sums to 1
          sheer_matrix_norm <- apply(sheer_matrix_norm, 2, function(col) { col / sum(col) })
          sheer_matrix_norm[sheer_matrix_norm < 0 | is.na(sheer_matrix_norm)] <- 0
          sheer_matrix_norm[is.nan(sheer_matrix_norm)] <- 0
          
          colnames(sheer_matrix_norm) <- cell_type_list
          rownames(sheer_matrix_norm) <- cell_type_list
        }
        print(sheer_matrix_norm)
        # Store the sheer matrix for the current depth in the list (name format: "depth_current_depth")
        all_sheer_matrix_list[[paste0("depth_", current_depth)]] <<- sheer_matrix_norm
      })
      
      sum_matrix <- Reduce("+", all_sheer_matrix_list)
      mean_matrix <- sum_matrix / length(all_sheer_matrix_list)
      
      sheer_norm <- data.frame()
      if(length(mean_matrix) != 0){
        #print(paste(file_path, "Null"))
        sheer_mean_matrix <- apply(mean_matrix, 2, function(col) { col / sum(col) })
        ###Perform optimization using the L-BFGS-B method
        #mean_matrix_flat <- as.vector(mean_matrix)
        #Bound_matrix <- matrix(Inf, nrow = length(cell_type_list), ncol = length(cell_type_list))
        #upper_bound <- as.vector(Bound_matrix)
        
        #optim_result <- optim(mean_matrix_flat,
        #                      calculate_transition_matrix_error,
        #                      observed_matrices = all_sheer_matrix_list,
        #                      matrix_size = nrow(mean_matrix),
        #                      type_list = cell_type_list,
        #                      upper = upper_bound,
        #                      method = "L-BFGS-B")
        
        #sheer_mean_matrix <- matrix(optim_result$par, nrow = nrow(mean_matrix), byrow = FALSE)
        #colnames(sheer_mean_matrix) <- cell_type_list
        #rownames(sheer_mean_matrix) <- cell_type_list
        #sheer_mean_matrix[sheer_mean_matrix < 0 | is.na(sheer_mean_matrix)] <- 0
        #sheer_mean_matrix <- apply(sheer_mean_matrix, 2, function(col) { col / sum(col) })
        #sheer_mean_matrix <- replace(sheer_mean_matrix, is.na(sheer_mean_matrix), 0)
        
        ##flatten matrix into long-form df
        combinations <- expand.grid(start_cell = colnames(sheer_mean_matrix),
                                    end_cell   = rownames(sheer_mean_matrix),
                                    stringsAsFactors = FALSE)
        
        sheer_norm <- lapply(seq_len(nrow(combinations)),function(i){
          start_cell_i <- combinations$start_cell[i]
          end_cell_i   <- combinations$end_cell[i]
          value_i <- sheer_mean_matrix[end_cell_i, start_cell_i]
          res <- data.frame(start_cell        = start_cell_i,
                            end_cell          = end_cell_i,
                            type_combined_new = paste(start_cell_i, end_cell_i, sep = "_"),
                            sheer.T = value_i)
          return(res)
        }) %>% bind_rows() %>% mutate(gene=sel.gene,file=file_path)
      }
      return(sheer_norm)
