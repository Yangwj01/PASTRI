...
def extractBC_UMI(read, P2, P4, mismatch=2):
    ...
    BC_UMI_fwd = "(?e)(?P<p2>%s){e<=%d}(?P<cellUMI>.{10})(?P<raw_seq>.*)(?P<UMI>.{12})(?P<BC>.{16})(?P<p4>%s){e<=%d}" % (P2, mismatch, P4, mismatch)
    BC_UMI_rev = "(?e)(?P<p4>%s){e<=%d}(?P<BC>.{16})(?P<UMI>.{12})(?P<raw_seq>.*)(?P<cellUMI>.{10})(?P<p2>%s){e<=%d}" % (seqre(seqco(P4)), mismatch, seqre(seqco(P2)), mismatch)
    ...
    if fwd_match:
        ...
        cellUMI = fwd_match.groupdict()["cellUMI"]
        return BC, UMI, raw_seq, cellUMI
    elif rev_match:
        BC = rev_match.groupdict()["BC"]
        UMI = rev_match.groupdict()["UMI"]
        raw_seq = seqre(seqco(rev_match.groupdict()["raw_seq"]))
        cellUMI = seqre(seqco(rev_match.groupdict()["cellUMI"]))
        return BC, UMI, raw_seq, cellUMI
    else:
        return None
...
def filterCCS(header, read, P2, P4, FP, RP, polyAwin=6, pAmismatch=1, mismatch=2):
    ...
    if trimP2P4_result:
        BC, UMI, raw_seq, cellUMI = trimP2P4_result
        ...
        if rType == "both":
            name = "zwID={} BC={} UMI={} cellUMI={} P2P4 FPRP".format(zwID, BC, UMI, cellUMI)
            return "pass", name, final_seq, zwID, BC, UMI, cellUMI
        elif rType == "fwd":
            name = "zwID={} BC={} UMI={} cellUMI={} P2P4 FP".format(zwID, BC, UMI, cellUMI)
            return "pass", name, final_seq, zwID, BC, UMI, cellUMI
        elif rType == "rev":
            name = "zwID={} BC={} UMI={} cellUMI={} P2P4 RP".format(zwID, BC, UMI, cellUMI)
            return "pass", name, final_seq, zwID, BC, UMI, cellUMI
        elif rType == "noMatch":
            name = "zwID={} BC={} UMI={} cellUMI={} P2P4 noMatch".format(zwID, BC, UMI, cellUMI)
            return "pass", name, final_seq, zwID, BC, UMI, cellUMI
    else:
        return "nope", header.strip(), read.strip(), zwID, "noBC", "noUMI", "nocellUMI"

def mycallback(results):
    filre, header, seq, zwid, BC, UMI, cellUMI = results
    if filre == "pass" and seq != "":
        ...
        outNoPass.write(">{}\n{}\n".format(header, seq))
        outNoPass.flush()
    outSta.write("{}\t{}\t{}\t{}\n".format(zwid, BC, UMI, cellUMI, filre))
    outSta.flush()
...
if __name__ == "__main__":
    ...
    outSta.write("{}\t{}\t{}\t{}\n".format("zwID", "BC", "UMI", "cellUMI", "pass_filter"))
