...
```
tree_file_path <- "./demo/datasets/extdata/HSC_1_B7.nwk"
node_depth_df <- calculate_lca_depths(tree.path = tree_file_path)
```
...
```
cell_file_path <- "./demo/datasets/extdata/HSC_1_B7_cellInfo.csv"
cell_info <- read.csv(cell_file_path, stringsAsFactors = FALSE)
```
...
```
Transition_matrix <- df_depth[[4]]$optimal_matrix %>% t()
```
Transition matrix, T: A square N×N matrix, where N is the number of cell states.The element in the i-th row and j-th column represents the probability, per cell cycle, that a cell in state i transitions to state j. Each row sums to 1.

## Contributing
### Contributors
Xiaochen Yu, https://github.com/yuxiaochen11/PASTRI.
